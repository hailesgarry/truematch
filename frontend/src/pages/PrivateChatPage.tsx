...
import "./ChatPage.css";
import { pickBubbleColor, DEFAULT_BUBBLE_PALETTE } from "../utils/bubbles";
...
const PrivateChatPage: React.FC = () => {
  ...
  const peerName = React.useMemo(() => decodeURIComponent(peer || ""), [peer]);
  const peerAvatar = getAvatar(peerName) || null;
  // defined earlier
  const {
    isConnected,
    ensureConnected,
  ...
  useEffect(() => {
    ...
    // Derive dmId from my username + peer param (case-insensitive id)
  ...
  const dmId =
    activeDmId ||
    (peer ? directRoomIdFor(username || "", decodeURIComponent(peer)) : null);
  const currentMessages = dmId ? messages[dmId] || [] : [];
    ...
    <div className="flex flex-col h-screen">
        ...
        <div className="flex items-center gap-4 px-3 h-14 sm:h-16">
          ...
          <div className="relative w-10 h-10">
            {peerAvatar ? (
              ...
              <div className="w-10 h-10 rounded-full bg-gray-200 text-gray-700 flex items-center justify-center text-sm font-semibold select-none">
      ...
      <div className="flex-1 overflow-y-auto">
        <div ref={listRef} className="flex flex-col min-h-full px-4 pt-4">
                ...
                <div className="flex">
                  ...
                  >
                    <div className="max-w-[75%]">
                      {!(m as any).deleted &&
                        (() => {
                          const map = ((m as any).reactions || {}) as Record<
                            string,
                            any
                          >;
                          ...
                          // Order unique emojis by most recent timestamp
                          const byEmoji = new Map<string, number>();
                          for (const r of entries) {
                            const prev = byEmoji.get(r.emoji) || 0;
                            if (!prev || (r.at || 0) > prev)
                              byEmoji.set(r.emoji, r.at || 0);
                          }
                          const orderedEmojis = Array.from(byEmoji.entries())
                            .sort((a, b) => b[1] - a[1])
                            .map(([e]) => e);
                          return (
                            <div
          ...
          {isConnected && dmId && currentMessages.length === 0 && (
      ...
      <div className=" px-4 py-3">
        {editingMessage && (
          <div className="mb-3 px-3 py-2 rounded-md bg-gray-100 relative flex flex-col gap-1">
            ...
            {/* Content: allow thumbnails + clamp text to two lines */}
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              {/* Composer input inside a rounded container, centered vertically via flex */}
              <div className="relative w-full border rounded-full pl-8 pr-28 py-2 bg-white flex items-center">
                ...
                <div className="pointer-events-none absolute inset-y-0 right-2 flex items-center gap-0">
                  <button
                    ...
                    className="pointer-events-auto p-1 text-gray-500"
                  ...
                  <button
                    ...
                    className="pointer-events-auto text-gray-500 px-1.5 py-1 focus:outline-none"
                    onMouseDown={(e) => e.preventDefault()}
                    onClick={() => {
                      if (inputRef.current) {
                        setCursorPos(
