...
@router.post("/dating/likes")
async def add_like(body: Dict):
    db = get_db()
    from_user = (body.get("from") or "").strip().lower()
    to_user = (body.get("to") or "").strip().lower()
    if not from_user or not to_user or from_user == to_user:
        raise HTTPException(status_code=400, detail="invalid like input")
    at = int(body.get("at") or __import__("time").time() * 1000)
    # upsert by pair
    await db["likes"].update_one(
        {"fromLc": from_user, "toLc": to_user},
        {"$set": {"fromLc": from_user, "toLc": to_user, "at": at}},
        upsert=True,
    )
    # Invalidate small caches
    try:
        ...
        pass
    return {"from": from_user, "to": to_user, "at": at}


@router.delete("/dating/likes")
async def remove_like(body: Dict):
    db = get_db()
    from_user = (body.get("from") or "").strip().lower()
    to_user = (body.get("to") or "").strip().lower()
    if not from_user or not to_user:
        raise HTTPException(status_code=400, detail="invalid like input")
    res = await db["likes"].delete_one({"fromLc": from_user, "toLc": to_user})
    try:
        await local_cache.delete_prefix(f"likes:incoming:{to_user}")
        await local_cache.delete_prefix(f"likes:outgoing:{from_user}")
        await publish_invalidate(f"likes:incoming:{to_user}")
...
@router.get("/dating/likes/incoming")
async def incoming_likes(user: str, request: Request = None, response: Response = None):
    db = get_db()
    u = (user or "").strip().lower()
    if not u:
        return []
    cache_key = f"likes:incoming:{u}"
    inm = request.headers.get("if-none-match") if request else None
    ...
    docs = (
        await db["likes"].find({"toLc": u}).sort("at", -1).to_list(length=5000)
    )
    out = [{"from": d.get("fromLc"), "at": d.get("at") or 0} for d in docs]
    await local_cache.set(cache_key, out, ttl_seconds=20)
    if response is not None:
        try:
...
@router.get("/dating/likes/outgoing")
async def outgoing_likes(user: str, request: Request = None, response: Response = None):
    db = get_db()
    u = (user or "").strip().lower()
    if not u:
        return []
    cache_key = f"likes:outgoing:{u}"
    inm = request.headers.get("if-none-match") if request else None
    ...
    docs = (
        await db["likes"].find({"fromLc": u}).sort("at", -1).to_list(length=5000)
    )
    out = [{"to": d.get("toLc"), "at": d.get("at") or 0} for d in docs]
    await local_cache.set(cache_key, out, ttl_seconds=20)
    if response is not None:
        try:
            raw = json.dumps(out, separators=(",", ":"), sort_keys=True)
